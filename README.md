...
├── config/
│   ├── crd/bases/                    # CRD manifest（controller-gen 生成）
...
# 生成 deepcopy / CRD
.\bin\controller-gen.exe object:headerFile="hack/boilerplate.go.txt" paths="./api/v1alpha1/..."
.\bin\controller-gen.exe crd paths="./api/v1alpha1/..." output:crd:artifacts:config=config/crd/bases
...
本地调试可使用 [cert-manager](https://cert-manager.io/) 的替代方案：
...
kubectl apply -f config/samples/pod_samples.yaml   # 创建 ml-platform / ai-training namespace
```
...
- ResourceQuota 对所有非系统 namespace 生效，大规模集群 reconcile 可能较慢（可通过 namespace label 过滤扩展）
...
```powershell
# 查看 Webhook 配置
kubectl get mutatingwebhookconfiguration gpu-isolation-mutating-webhook
kubectl get validatingwebhookconfiguration gpu-isolation-validating-webhook

# 非法 Pod（default namespace）应被拒绝
kubectl apply -f config/samples/pod_samples.yaml
# 观察 illegal-gpu-pod 创建失败
...
kubectl get pod test-gpu -n ml-platform -o yaml | findstr platform.example.com
```
